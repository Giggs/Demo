...
namespace FunBooksAndVideos.BusinessLogic.Tests.Models
{
    ...
    class PurchaseOrderTests
    {
        ...
        public void Setup()
        {
            ...
            _shippingSlipGeneratorMock.Setup(x => x.Generate(It.IsAny<IEnumerable<PhysicalProduct>>())).Returns(42);
            _mapperMock = new Mock<IMapper>();
            _mapperMock
                .Setup(x => x.Map<MembershipDAO>(It.IsAny<object>()))
                .Returns((object arg) =>
                {
                    if (arg is BookMembership)
                    {
                        return new BookMembershipDAO();
                    }
                    else if (arg is VideoMembership)
                    {
                        return new VideoMembershipDAO();
                    }
                    else if (arg is PremiumMembership)
                    {
        ...
        public void Process_WhenThereArePhysicalProductsAndMemberships_GeneratesShippingSlipAndActivatesMemberships()
        {
            ...
            _shippingSlipGeneratorMock.Verify(x =>
                x.Generate(It.IsAny<IEnumerable<PhysicalProduct>>()), Times.Once);
            _customerRepositoryMock.Verify(x =>
                x.ActivateMembership(It.IsAny<long>(), It.IsAny<MembershipDAO>()), Times.Once());
